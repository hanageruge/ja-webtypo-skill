...
- ヒーローコピーが「私たちは、足の/専門家です」のように変な位置で折り返される
- ボタンの日本語が2行に割れる
- カードタイトルが半端な場所で改行される
- `、` や `。` が行頭に来てしまう
- スマホで価格表やフォームがはみ出す
...
このスキルは「日常的なWeb制作の日本語タイポ」に集中しています。次のような特殊用途は範囲外です:

- **フォント見本ページ（specimen）の最適化** — 専用スキルを分離予定
- **縦書きレイアウト（`writing-mode: vertical-rl`）** — 別ドメイン
- **印刷用 CSS（@media print の日本語禁則）** — 別スキル候補
- **ルビ（`<ruby>`）の細かい挙動** — 別スキル候補
- **特定業種向けのレイアウトチェックリスト** — 別スキルで提供推奨
...
issue / PR は歓迎します。特に次のフィードバックが助かります:

- 追加すべき改行制御パターン
- 他フレームワーク（Qwik / SolidJS / Remix 等）での適用例
- 各 AI ツールでのインストール手順の改善
- ブラウザ互換性情報の更新
